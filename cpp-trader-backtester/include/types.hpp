...
struct alignas(64) Order {
    ...
    OrderStatus status;
    uint32_t user_id;
    SymbolId symbol_id;  // 2 bytes — fits in padding, keeps struct at 64 bytes

    Order() = default;
    Order(OrderId id_, Price price_, Quantity qty_, Timestamp ts_,
          Side side_, OrderType type_, uint32_t user_, SymbolId symbol_id_ = 0)
        : id(id_), price(price_), quantity(qty_), filled(0),
          initial_quantity(qty_), timestamp(ts_), side(side_), type(type_),
          status(OrderStatus::PENDING), user_id(user_), symbol_id(symbol_id_) {}
