...
import psi_patch_agent  # noqa: E402


class PsiPatchAgentCliTests(unittest.TestCase):
    def test_direct_edit_prompt_forbids_local_build_artifacts(self) -> None:
        prompt = psi_patch_agent.build_direct_edit_prompt(
            {
    ...
    def test_claude_cli_uses_windows_cmd_fallback(self) -> None:
        with tempfile.TemporaryDirectory(prefix="psi_patch_agent_") as raw_dir:
            ...
            with (
                mock.patch.object(
                    psi_patch_agent.shutil,
                    "which",
                    side_effect=lambda name: "C:/Users/test/AppData/Roaming/npm/claude.cmd"
                    if name == "claude.cmd"
                    else None,
                ),
                mock.patch.object(psi_patch_agent.subprocess, "run", return_value=completed) as run_mock,
            ):
                response = psi_patch_agent.call_claude_cli("prompt", workspace)
            ...
            self.assertFalse((workspace / ".psi_patch_prompt.txt").exists())

    def test_claude_cli_reports_missing_binary(self) -> None:
        with tempfile.TemporaryDirectory(prefix="psi_patch_agent_") as raw_dir:
            workspace = Path(raw_dir)
            with mock.patch.object(psi_patch_agent.shutil, "which", return_value=None):
                response = psi_patch_agent.call_claude_cli("prompt", workspace)
    ...
    def test_claude_cli_empty_response_is_reported(self) -> None:
        with tempfile.TemporaryDirectory(prefix="psi_patch_agent_") as raw_dir:
            ...
            with (
                mock.patch.object(psi_patch_agent.shutil, "which", return_value="claude.cmd"),
                mock.patch.object(psi_patch_agent.subprocess, "run", return_value=completed),
            ):
                response = psi_patch_agent.call_claude_cli("prompt", workspace)
    ...
    def test_codex_cli_direct_edit_requires_workspace_changes(self) -> None:
        with tempfile.TemporaryDirectory(prefix="psi_patch_agent_") as raw_dir:
            ...
            with (
                mock.patch.object(psi_patch_agent.shutil, "which", return_value="codex.cmd"),
                mock.patch.object(psi_patch_agent.subprocess, "run", return_value=completed) as run_mock,
                mock.patch.object(psi_patch_agent, "workspace_has_changes", return_value=True),
            ):
                ok = psi_patch_agent.call_codex_cli("prompt", workspace, run_dir, "candidate")
    ...
    def test_codex_cli_direct_edit_fails_when_no_diff(self) -> None:
        with tempfile.TemporaryDirectory(prefix="psi_patch_agent_") as raw_dir:
            ...
            with (
                mock.patch.object(psi_patch_agent.shutil, "which", return_value="codex.cmd"),
                mock.patch.object(psi_patch_agent.subprocess, "run", return_value=completed),
                mock.patch.object(psi_patch_agent, "workspace_has_changes", return_value=False),
            ):
                ok = psi_patch_agent.call_codex_cli("prompt", workspace, run_dir, "candidate")
