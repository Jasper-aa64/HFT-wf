...
1. **Overclock Mode** — multi-agent adversarial review with executable gates. The slide pattern.
...
> Talking to one coding agent in a chat window is not Overclock Mode. Without a second role, a deterministic executor, and an acceptance gate, there is no quality system — only a single LLM with edit access.

---

## 1. Overclock Mode: Multi-Agent Adversarial Review
