...
- Each contains: `builder_prompt.md`, `builder.log`, `patch.diff`, `eval.log`, `critic.md`, `decision.md`

Final output:
- `final_decision.md` with verdict: APPROVE or ESCALATE
...
```
scripts/
  overclock_cli_loop.sh       # Main loop (retry loop v1)
  evaluators/
    evaluate_safe_divide.sh   # Toy evaluator
    evaluate_safe_add.sh      # Test evaluator for retry
    evaluate_safe_multiply.sh # Test evaluator for retry
    evaluate_impossible.sh    # Test evaluator (always fails)
...
overclock_runs/               # Run history
  ...
  test-attempt1-approve-brief.md   # Test brief: should pass on attempt 1
  test-retry-on-reject-brief.md    # Test brief: should retry and pass
  test-escalate-brief.md           # Test brief: should ESCALATE after 3 failures
...
```bash
# Run Overclock with retry loop (default 3 attempts)
./scripts/overclock_cli_loop.sh <brief.md>

# With custom max attempts
./scripts/overclock_cli_loop.sh --max-attempts 5 <brief.md>

# With auto-apply on APPROVE
./scripts/overclock_cli_loop.sh --apply <brief.md>

# Show help
./scripts/overclock_cli_loop.sh --help
...
```text
MVP Status: Retry Loop v1 implemented ✓

Ready for:
- Validation tests (attempt-1 APPROVE, retry-on-REJECT, ESCALATE)
...
- Persistent worktree across attempts
```
