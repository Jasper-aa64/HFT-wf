...
```text
Isolation (P2):
  ✓ Runs in git worktree (branch: overclock/<timestamp>)
  ✓ Main repo untouched until APPROVE
  ✓ REJECT → worktree deleted, no cleanup needed

Pre-flight checks:
  ✓ Git repository required
  ✓ CLI availability verified
  ✓ (No clean working dir required — worktree isolation)

Brief format:
  YAML frontmatter with eval_script (must be under scripts/)
  allowed_files with glob pattern support

Claude Code:
  --print --allowedTools "Read,Edit,Write" (no Bash)

Codex:
  --skip-git-repo-check
  Structured output: VERDICT: APPROVE/REJECT

Security:
  ✓ eval_script must be under scripts/
  ✓ allowed_files machine-verified
  ✓ No shell injection via brief
```
...
```text
.git/worktrees/overclock-<timestamp>/  # Isolated workspace
branch: overclock/<timestamp>           # Deleted after run
```

APPROVE flow:

1. Patch applied to main worktree
2. New files copied to main worktree
3. Worktree cleaned up
4. Human reviews and commits

REJECT flow:

1. Worktree deleted (no cleanup in main repo needed)
2. Decision package saved for review
